...
const tests = [`abcdef`,`bababc`,`abbcde`,`abcccd`,`aabcdd`,`abcdee`,`ababab`];

function repeatedTwoAndOrThree(string) {
    ...
    let two = 0;
    let three = 0;
    if (seen.some(freq => freq === 2))
        two = 1;
    if (seen.some(freq => freq === 3))
        three = 1;
    return [two, three];
}

tests.forEach(str => {
    console.log(`Test \'${str}\': ${repeatedTwoAndOrThree(str)}`);
});

const strings = input.split('\n');

const part1 = strings
.map(str => repeatedTwoAndOrThree(str))
.reduce((total, next) => {
    total[0] += next[0];
    total[1] += next[1];
    return total;
});
console.log(`Part 1 Answer: ${part1[0] * part1[1]}`);
...
function sameLettersFor1Apart(strings) {
    for (let i = 0; i < strings.length; ++i) {
        for (let j = 0; j < strings.length; ++j) {
            ...
            let diff = 0;
            let sameLetters = '';
            for (let letter = 0; letter < strings[i].length; ++letter) {
                if (strings[i][letter] !== strings[j][letter])
                    ++diff;
                else
                    sameLetters += strings[i][letter];
            }
            if (diff === 1)
                return sameLetters;
        }
...
console.log(`Test for part 2: ${sameLettersFor1Apart(test2.split('\n'))}`);
console.log(`Part 2 Answer: ${sameLettersFor1Apart(strings)}`);
