...
function parse(str) {
    let formatted = str.replace(/1518-|[\[\]]|\d\d:|#/g,"");
    formatted = formatted.replace(/-/g, " ");
    // e.g. 10 08 47 Guard 1601 begins shift
    const args = formatted.trim().split(' ');
    let results = new Object();
    results.month = parseInt(args[0]);
    results.day = parseInt(args[1]);
    results.minutes = parseInt(args[2]);
    results.awake = (args[3] === "wakes");
    results.asleep = (args[3] === 'falls');
    results.id = (args[3] === 'Guard') ? parseInt(args[4]) : null;
    return results;
}

const guards = new Map();
parsed = lines.map(line => parse(line));
...
// Initialize guards
parsed.forEach(line => {
    if (line.id != null && !guards.has(line.id)) {
        guards.set(line.id, new Object());
        guards.get(line.id).totalAsleep = 0;
        guards.get(line.id).minutes = new Array(60);
        for (let i = 0; i < 60; ++i)
            guards.get(line.id).minutes[i] = 0;
    }
});

// Calculate when guards were asleep and sum their total time slept
parsed.forEach(line => {
    if (line.id != null) {
        if (updating != null && minuteAsleep != null) {
            guards.get(updating).totalAsleep += 60 - minuteAsleep;
            for (let i = minuteAsleep; i < 60; ++i)
                ++guards.get(updating).minutes[i];
        }
        minuteAwake = line.minutes;
        minuteAsleep = null;
        updating = line.id;
    }
    if (line.asleep) {
        minuteAsleep = line.minutes;
        minuteAwake = null;
    }
    if (line.awake) {
        if (minuteAsleep != null) {
            guards.get(updating).totalAsleep += (line.minutes - minuteAsleep);
            for (let i = minuteAsleep; i < line.minutes; ++i)
                ++guards.get(updating).minutes[i];
        }
        minuteAwake = line.minutes;
        minuteAsleep = null;
    }
    lastMonth = line.month;
    lastDay = line.day;
});

const guardStats = new Array();
for (let guard of guards.entries()) {
    guardStats.push(guard);
}

// Determine the guard who slept the most and when he slept the most
guardStats.sort((a,b) => b[1].totalAsleep - a[1].totalAsleep);
const guardMostAsleep = guardStats[0][0];
...
// Determine the guard who slept the most frequently on a particular minute
// and what minute that was
let minuteMostFrequentAsleep = null;
let frequency = -1;
let guardMostFrequentAsleep = null;
for (let guard of guardStats) {
    const id = guard[0];
    const minutes = guard[1].minutes;
    for (let i = 0; i < 60; ++i) {
        if (minutes[i] > frequency) {
            guardMostFrequentAsleep = id;
            minuteMostFrequentAsleep = i;
            frequency = minutes[i];
        }
    }
}
const part2 = minuteMostFrequentAsleep * guardMostFrequentAsleep;
